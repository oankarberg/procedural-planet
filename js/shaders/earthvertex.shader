...
float turbulence( vec3 p,out vec3 gradient) {
    float w = 100.0;
    float t =0.0;
    for (float f = 1.0 ; f <= 10.0 ; f++ ){
        float power = pow( 2.0, f );
        vec3 gradTemp = vec3(0.0);
        t += abs( snoise( vec3( power * p ),  gradTemp));
        gradient  += gradTemp * power;
    }
    return t;
}
...
void main() {
    ...
    if(elevation < 0.0){ 
      ...
      gradTemp = vec3(0.0);
      elevation += 0.005 * multiplier * (snoise(position * 10.0 / multiplier,gradTemp));
      grad += gradTemp * 0.005 * 10.0; // update Gradient
    }
    displ = elevation;
    //Update new position and view position
    vec3 newPosition = position + normal * displ * bumpNoise;
